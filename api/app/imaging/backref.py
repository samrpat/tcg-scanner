...
# The photograph the reference was built from. Card-back artwork, owned by Nintendo /
# Creatures / GAME FREAK, and therefore **not shipped in a distributed build** — see
# `.dockerignore` and `docs/DISTRIBUTION.md`.
REFERENCE_PATH = _ASSETS / "pokemon-back.jpg"

# What is shipped instead: the ORB keypoints and descriptors computed from it, plus the
# dimensions they were computed at. Registration needs nothing else — the reference pixels are
# only ever used for `ref_image.shape`.
#
# A set of binary feature descriptors is not a reproduction of the artwork and cannot be turned
# back into it; what it encodes is "there is a corner here, at this scale, with this local
# gradient pattern". That makes it distributable where a 156 KB photograph of somebody else's
# card is not.
#
# Rebuild it with `make bake-reference` after replacing the photograph.
DESCRIPTORS_PATH = _ASSETS / "pokemon-back.npz"
...
def bake_descriptors(image_path: Path = REFERENCE_PATH, out: Path = DESCRIPTORS_PATH) -> dict:
    """Compute the reference's ORB features and write them to disk.

    Run once, whenever the reference photograph changes, so that a build can ship the features
    without shipping the artwork they came from.
    """
    image = cv2.imread(str(image_path), cv2.IMREAD_GRAYSCALE)
    if image is None:
        raise FileNotFoundError(f"no reference image at {image_path}")

    orb = cv2.ORB_create(nfeatures=MAX_FEATURES)
    keypoints, descriptors = orb.detectAndCompute(image, None)
    if descriptors is None or len(keypoints) < MIN_MATCHES:
        raise ValueError(f"only {len(keypoints or [])} features; the reference is unusable")

    np.savez_compressed(
        out,
        # cv2.KeyPoint does not pickle, so each field travels as a column.
        ...
        shape=np.array(image.shape[:2], dtype=np.int32),
    )
    return {
        "written": str(out),
        "features": len(keypoints),
        "reference_size": list(image.shape[:2]),
        "bytes": out.stat().st_size,
    }


@lru_cache(maxsize=1)
def _reference() -> tuple[tuple[int, int], tuple, np.ndarray] | None:
    """The reference's dimensions, ORB keypoints and descriptors. Once per process.

    Prefers the baked descriptors, because that is what a distributed build has. Falls back to
    computing them from the photograph, which is what a working copy has — the two produce
    identical features, so which one is present changes nothing but what is on disk.
    """
    if DESCRIPTORS_PATH.exists():
        with np.load(DESCRIPTORS_PATH) as data:
            keypoints = tuple(
                cv2.KeyPoint(
                    x=float(pt[0]),
                    y=float(pt[1]),
                ...
                for pt, size, angle, response, octave in zip(
                    data["points"],
                    data["sizes"],
                    data["angles"],
                    data["responses"],
    ...
    if descriptors is None or len(keypoints) < MIN_MATCHES:
        return None
    return image.shape[:2], keypoints, descriptors
...
def register_back(image: np.ndarray) -> Registration | None:
    ...
    if reference is None or image is None or image.size == 0:
        return None
    ref_shape, ref_keypoints, ref_descriptors = reference
