...
class Settings(BaseSettings):
    ...
    # What this build is. The Dockerfile sets TCG_VERSION from the release tag; a working copy
    # says "dev". Aliased rather than named `VERSION`, which is far too generic a name to claim
    # out of the ambient environment.
    version: str = Field("dev", validation_alias="TCG_VERSION")

    # ── Authentication ──────────────────────────────────────────────────────
    # On by default, and the default is the whole point: this application shipped for months
    # with every endpoint open to anything that could reach the port, including the ones that
    # delete inventory. Turning it off is a deliberate choice for a machine on a network you
    # control completely — it is reported by /health/detail and shown as a banner in the UI so
    # it can never be the quiet state.
    auth_required: bool = True
    # A month. Long because the phone doing the scanning must not ask for a password in the
    # middle of a pile, and revocation exists for the case that matters (a lost device).
    auth_session_days: int = 30
    auth_cookie_name: str = "tcg_session"
    # Failed logins allowed from one address inside the window before it is refused. Guards a
    # single-password front door against someone with a wordlist and a weekend.
