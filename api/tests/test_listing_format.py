...
def test_batch_names_read_as_dates_and_numbers():
    ...
    from app.routers.sessions import _generated_name

    source = inspect.getsource(_generated_name)
    assert "Batch" in source
    assert "isoformat" in source
    # Taken from the batch's own start, not the clock: clearing the name of last week's batch
    # must restore last week's date.
    assert "when.date()" in source
