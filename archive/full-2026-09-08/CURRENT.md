...
**Last verified:** 2026-09-04, against the running stack.

**The whole path works end to end**: scan → identify → approve → condition → price → eBay
upload file with photographs attached. What remains is polish and volume, not missing pieces.

| | |
|---|---|
| Tests | **341 passing** (`make test`) |
| Endpoint smoke checks | **38 passing** (`make smoke`) |
| Lint | `ruff check` clean; TypeScript and production build clean |
| Migrations | **0001 → 0017**, 21 tables |
| Catalogue | 218 sets, 23,544 cards, 35,495 variants |
| API routes | 70 |
| Stack memory, idle | **373 MB** (postgres 184, api 101, worker 64, redis 14, web 5) |
...
**Empty and ready.** The 33 development cards were cleared on 2026-09-04 for a real run
(D-132); they are in `data/backups/20260904-075318` if anything is wanted back. The open
scanning session is "First real batch", and the next scan allocates `CARD-000001`.
