...
from app.models import Card, CardVariant, Image, InventoryItem, Review, User
...
async def current_user(session: AsyncSession = Depends(get_session)) -> User:
    """Single-user for now. `user_id` is threaded through everything already (spec §40), so
    this is the only place that needs to change when authentication arrives."""
    user = (
